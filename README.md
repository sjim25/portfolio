...
Playwright + TypeScript 기반 E2E 자동화 포트폴리오입니다. 외부 데모 사이트인 [Sauce Demo](https://www.saucedemo.com/)를 대상으로 POM, fixture, config, reporter, locator 전략, GitHub Actions CI 구성을 보여줍니다.
...
README 상단에 GitHub Actions Badge를 추가해 현재 E2E 테스트가 GitHub CI 환경에서 통과 중인지 직관적으로 확인할 수 있도록 했습니다.
...
테스트 대상은 외부 데모 사이트 [Sauce Demo](https://www.saucedemo.com/)이므로, 별도 로컬 서버를 띄울 필요 없이 아래 명령만으로 실행할 수 있습니다.
...
```bash
git clone https://github.com/sjim25/portfolio.git
cd portfolio
```
...
| 명령어 | 설명 |
| --- | --- |
| `npm run test:e2e` | Chromium, Firefox, WebKit 전체 E2E 테스트 실행 |
| `npm run test:e2e:chromium` | Chromium만 실행 (빠른 smoke test) |
| `npm run test:e2e:headed` | 브라우저 창을 띄워 실행 |
| `npm run test:e2e:ui` | Playwright UI 모드로 실행 |
| `npm run test:e2e:debug` | 디버그 모드로 실행 |

빠르게 확인하려면 Chromium smoke test부터 실행하는 것을 권장합니다:

```bash
npm run test:e2e:chromium
```

![Chromium E2E 테스트 실행 결과](assets/screenshots/test-result.png)

특정 spec 파일만 실행하려면 Playwright CLI 옵션을 함께 사용할 수 있습니다:
...
테스트 실행 후 HTML 리포트는 다음 명령어로 확인할 수 있습니다:

```bash
npm run report
```

![Playwright HTML 리포트](assets/screenshots/playwright-report.png)
...
![체크아웃 완료 화면](assets/screenshots/test-ui.png)
...
![GitHub Actions Playwright E2E 성공 화면](assets/screenshots/github-actions-ci.png)
